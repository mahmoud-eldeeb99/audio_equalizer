...
class popWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.zoomout.clicked.connect(lambda: self.zoom(0.5))  #######
        self.ui.zoomin.clicked.connect(lambda: self.zoom(2))
        ...
        for iteration in range(0, 10):
            self.slider[iteration].valueChanged.connect(lambda: self.draw_the_new_signal_and_spectrogram())
    ...
    def create_pdf(self):  ##take pdf to spectrogram of chaneel 1
        ...
        exporter.export(f'{PLOT_DIR}/2.jpg')
    ...
    def zoom(self, x):
        self.ui.o_signal.plotItem.getViewBox().scaleBy((x, x))
        self.ui.m_signal.plotItem.getViewBox().scaleBy((x, x))
        self.ui.o_spect.plotItem.getViewBox().scaleBy((x, x))
        self.ui.m_spect.plotItem.getViewBox().scaleBy((x, x))
    ...
    def load_audio_feil(self):
        ...
        self.ui.o_signal.plot(self.time[:1000], data[:1000])
        self.ui.o_signal.plotItem.setLimits(xMin=0, xMax=(1 / self.sampling_rate) * self.num_samples,
                                            yMin=min(self.data), yMax=max(self.data))
    ...
    def draw_the_new_signal_and_spectrogram(self):
        ...
        for x in range(0, 10):
            slider_valeuse.append(self.slider[x].value())

        band = 2000

        if (
                self.ui.high_freq_slider.value() == 0 and self.ui.low_freq_slider.value() == 0):  ##for multiplying hight and low freqency by zero
            ...
            for x in range(1, 6):
                if x == 5:
                    self.new_freqencyes = np.concatenate((self.new_freqencyes, self.frequencies[(x - 1) * band:] * 0),
                                                         axis=0)
                else:
                    self.new_freqencyes = np.concatenate(
                        (self.new_freqencyes, self.frequencies[(x - 1) * band:band * x] * slider_valeuse[x - 1]),
                        axis=0)
            ...
            for x in range(1, 11):
                if x <= 5:
                    self.new_freqencyes = np.concatenate(
                        (self.new_freqencyes, self.frequencies[(x - 1) * band:band * x] * 0), axis=0)


                elif x == 10:
                    self.new_freqencyes = np.concatenate(
                        (self.new_freqencyes, self.frequencies[(x - 1) * band:] * slider_valeuse[x - 1]), axis=0)
        ...
        self.ui.m_signal.plotItem.setLimits(xMin=0, xMax=(1 / self.sampling_rate) * self.num_samples,
                                            yMin=min(self.new_signal), yMax=max(self.new_signal))
        self.ui.m_signal.clear()
        self.ui.m_signal.plot(self.time[:1000], self.new_signal[:1000])
